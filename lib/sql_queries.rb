...
#"SELECT users.name, users.age, pledges.amount FROM users JOIN users ON users users.id = pledges.users_id GROUP BY users.name"
...
#"SELECT projects.title, over =projects.funding_goal-SUM(pledges.amount) FROM projects WHERE over > projects.funding_goal;"
  "SELECT projects.title, (SUM(pledges.amount)-projects.funding_goal) AS over FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING over >=0;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
#"SELECT users.name, SUM(pledges.amount) AS total FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY users.name, total ASC"
...
def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
  # Why did the above use sum but not this one?
  #"SELECT projects.category, SUM(pledges.amount) FROM projects JOIN pledges ON projects.id = pledges.poject_id WHERE projects.category == 'music;"
...
def selects_the_category_name_and_the_sum_total_of_the_all_its_pledges_for_the_books_category
  #Why do you need to have group by at the end?
